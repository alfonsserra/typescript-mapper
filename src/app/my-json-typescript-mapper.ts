...
import { isArrayOrArrayClass, isPrimitiveOrPrimitiveClass, isTargetType } from './util/utils';
  ...
  Object.keys(instance)
    .forEach((key: string) => {
      ...
      if (decoratorMetaData && decoratorMetaData.customConverter) {
        putProperty(instance, key, decoratorMetaData.customConverter.fromJson(json[decoratorMetaData.name || key]));
      } else {
        putProperty(instance, key, decoratorMetaData ? mapFromJson(decoratorMetaData, instance, json, key) : json[key]);
      }
    });
...
export function putProperty(object: any, key: string, value: any) {
  object[key] = value;
}
